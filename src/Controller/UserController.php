...
final class UserController extends AbstractController
{
    ...
    */
    ...
    public function showPostProfile(Post $post)
    {
        $userPosts = $post->getMyUser();

        return $this->render('post/show/showProfile.html.twig', [
            'userPosts' => $userPosts
        ]);
    }
        ...
        if ($editUserForm->isSubmitted() && $editUserForm->isValid()) {
            ...
            if ($newProfilePicture) {
                ...
                if (is_dir($oldProfilePicture)) {
                    ...
                    un identifiant unique grâce à la fonction "uniqid()", puis je précise l'extension de l'image grâce à la
                    fonction "guessExtension()" appliquée sur la variable $image. */
                    $newFilename = $safeFilename . '-' . uniqid() . '.' . $newProfilePicture->guessExtension();
        ...
        if ($editUserForm->isSubmitted() && $editUserForm->isValid()) {
            ...
            if ($newBanner) {
                ...
                if (is_dir($oldBanner)) {
                    ...
                    $originalFilename = pathinfo($newBanner->getClientOriginalName(), PATHINFO_FILENAME);
                    $safeFilename = $slugger->slug($originalFilename);
